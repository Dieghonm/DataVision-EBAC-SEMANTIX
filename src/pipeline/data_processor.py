...
class DataProcessor:
    ...
    def load_dataset(self, source):
        ...
        Args:
            source (str): Nome do dataset ('iris', 'wine', 'breast_cancer')
    ...
    def preprocess_features(self, X_train, X_test):
        ...
        # 3. Seleção de features (se habilitado)
        if self.config['preprocessing']['feature_selection1']['enabled']: #aqui que aparece
    ...
    def _select_features(self, X_train, X_test, y_train):
        ...
        method = self.config['preprocessing']['feature_selection']['method']
        k = self.config['preprocessing']['feature_selection']['k']
        
        self.logger.info(f"Selecionando features com método: {method}")
